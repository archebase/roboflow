...
fn test_batched_vs_original_performance() {
    ...
    for batch_size in batch_sizes {
        ...
        println!(
            "  Batched BagSource:   {:.2}s ({:.2}x)",
            batched_time, speedup_batched
        );
        println!(
            "  Blocking BagSource:  {:.2}s ({:.2}x)",
            blocking_time, speedup_blocking
        );
    }
...
async fn run_original_bag_source() -> f64 {
    ...
    let mut source = BagSource::from_config(&source_config).expect("Failed to create BagSource");
