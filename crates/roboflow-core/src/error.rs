...
//!
//! Provides a unified error type that all roboflow crates can use.
//!
//! # Error Categories
//!
...
//! - **Storage**: Cloud storage errors (error codes: 6xxx) - string-based for extensibility
...
pub enum RoboflowError {
    ...
    /// Storage error (string-based for extensibility across crates)
    Storage {
        /// Storage backend (e.g., "s3", "oss", "local")
        backend: String,
        /// Error message
        message: String,
...
impl RoboflowError {
    ...
    pub fn is_retryable(&self) -> bool {
        match self {
            ...
            RoboflowError::Storage { retryable, .. } => *retryable,
            _ => false,
        }
    }
    ...
    pub fn category(&self) -> ErrorCategory {
        match self {
            ...
            RoboflowError::Storage { .. } => ErrorCategory::Storage,
        }
    }
...
/// Result type for roboflow operations.
...
mod tests {
    ...
    fn test_storage_error() {
        let err = RoboflowError::storage("s3", "connection refused", true);
