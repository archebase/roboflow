//! MCAP format implementation for the unified I/O layer.
//!
...
impl McapFormat {
    ...
    pub fn create_writer<P: AsRef<Path>>(
        ...
        let file = File::create(path_ref)
            .map_err(|e| CodecError::encode("McapFormat", format!("Failed to create file: {e}")))?;

        let buf_writer = std::io::BufWriter::new(file);

        let writer = crate::format::writer::ParallelMcapWriter::new(buf_writer).map_err(|e| {
            CodecError::encode("McapFormat", format!("Failed to create writer: {e}"))
        })?;

        Ok(Box::new(McapWriterWrapper {
            inner: writer,
        ...
        for result in results {
            ...
            if processed.chunk.message_count() > 0 {
                ...
                let static_chunk = unsafe {
                    std::mem::transmute::<MessageChunk<'_>, MessageChunk<'static>>(processed.chunk)
                };
...
/// MCAP writer wrapper.
///
/// Wraps the existing ParallelMcapWriter.
struct McapWriterWrapper {
    /// The existing custom MCAP writer
    inner: crate::format::writer::ParallelMcapWriter<std::io::BufWriter<std::fs::File>>,
    /// File path
    path: String,
    /// Map from topic to channel ID
    channel_map: HashMap<String, u16>,
    /// Next channel ID to assign
    next_channel_id: u16,
}

impl FormatWriter for McapWriterWrapper {
    fn path(&self) -> &str {
        &self.path
    }
    ...
    fn add_channel(
        ...
        if let Some(&id) = self.channel_map.get(topic) {
            return Ok(id);
        }
        ...
        self.channel_map.insert(topic.to_string(), id);
        Ok(id)
    }

    fn write(&mut self, _message: &RawMessage) -> Result<()> {
        Err(CodecError::unsupported(
            "Direct message writing not supported. Use write_chunk for parallel writing.",
        ))
    }
    ...
    fn finish(&mut self) -> Result<()> {
        self.inner
            .finish()
            .map_err(|e| CodecError::encode("McapWriter", e.to_string()))?;
        Ok(())
    }

    fn message_count(&self) -> u64 {
        self.inner.messages_written()
    }
