//! LeRobot conversion configuration.
//!
//! Parses TOML configuration for MCAP → LeRobot conversion.
...
impl LeRobotConfig {
    ...
    pub fn state_mappings(&self) -> Vec<&Mapping> {
        self.mappings
            .iter()
            .filter(|m| matches!(m.mapping_type, MappingType::State | MappingType::Action))
            .collect()
    }
}
...
pub struct Mapping {
    /// MCAP topic pattern
    pub topic: String,
    /// LeRobot feature path (e.g., "observation.camera_0")
    pub feature: String,
...
"#;
        ...
        let config: LeRobotConfig = toml::from_str(toml_content).unwrap();
...
"#;
        ...
        let result: Result<LeRobotConfig, _> = toml::from_str(toml_content);
