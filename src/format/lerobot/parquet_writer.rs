//! LeRobot Parquet + MP4 format writer.
//!
//! Writes LeRobot datasets in the v3.0 format:
...
/// Parquet + MP4 LeRobot dataset writer.
///
/// Creates LeRobot datasets compatible with v3.0 format:
/// - `data/` directory with Parquet shards
/// - `videos/` directory with MP4 shards
#[allow(dead_code)]
pub struct ParquetLeRobotWriter {
    episode_id: usize,
...
impl ParquetLeRobotWriter {
    ...
    pub fn write_from_mcap(
        ...
        #[cfg(not(feature = "lerobot-parquet"))]
        {
            ...
            Err(anyhow::anyhow!(
                "Parquet support not enabled. Add feature 'lerobot-parquet' to Cargo.toml"
            )
            .into())
        }
    }
    ...
    fn write_from_mcap_impl(
        ...
        println!("Converting MCAP to LeRobot Parquet+MP4 format");
        ...
        // Write metadata
        crate::format::lerobot::info::write_info_json(
            &self.output_dir,
            config,
...
mod tests {
    ...
    fn test_create_writer() {
        ...
        let writer = ParquetLeRobotWriter::create(&temp_dir, 0);
        assert!(writer.is_ok());
    }

    #[test]
    fn test_writer_has_correct_directories() {
        let temp_dir = std::env::temp_dir().join("lerobot_test");
        std::fs::remove_dir_all(&temp_dir).ok();
        std::fs::create_dir_all(&temp_dir).ok();

        let writer = ParquetLeRobotWriter::create(&temp_dir, 0).unwrap();
    ...
    fn test_image_shape_recording() {
        let temp_dir = std::env::temp_dir().join("lerobot_test2");
        std::fs::remove_dir_all(&temp_dir).ok();
        std::fs::create_dir_all(&temp_dir).ok();

        let mut writer = ParquetLeRobotWriter::create(&temp_dir, 0).unwrap();
