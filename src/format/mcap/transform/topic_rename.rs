...
impl WildcardTopicMapping {
    ...
    /// * `target` - Target pattern like "/robocodec/*"
...
impl TopicRenameTransform {
    ...
    /// - Pattern `/foo/*` with target `/robocodec/*`
    /// - Maps `/foo/upperlimb/joint_states` to `/robocodec/upperlimb/joint_states`
    ///
    /// # Arguments
    ///
    /// * `pattern` - Source pattern with wildcard (e.g., "/foo/*")
    /// * `target` - Target pattern with wildcard (e.g., "/robocodec/*")
    ///
    /// # Errors
    ///
...
mod tests {
    ...
    fn test_wildcard_mapping() {
        let mut transform = TopicRenameTransform::new();
        transform
            .add_wildcard_mapping("/foo/*", "/robocodec/*")
            .unwrap();

        // Exact match for wildcard pattern
        assert_eq!(
            transform.apply("/foo/upperlimb/joint_states"),
            Some("/robocodec/upperlimb/joint_states".to_string())
        );
        assert_eq!(
            transform.apply("/foo/sensor/camera"),
            Some("/robocodec/sensor/camera".to_string())
        );
