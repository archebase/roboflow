...
impl McapRewriter {
    ...
    ///
    /// ```ignore
    /// use robocodec::mcap::rewriter::{McapRewriter, RewriteOptions};
    /// use robocodec::transform::TransformBuilder;
    /// use std::collections::HashMap;
    ///
    /// // With transformations
    /// let mut topic_renames = HashMap::new();
    /// topic_renames.insert("/old_camera".to_string(), "/camera".to_string());
    ///
        ...
        P2: AsRef<Path>,
    {
        ...
        let mut mcap_writer = mcap::Writer::new(BufWriter::new(output_file)).map_err(|e| {
        ...
        // Extract boolean flags (can't clone RewriteOptions due to TransformPipeline)
        ...
        for result in &mut stream {
            ...
            let seq = {
                let entry = self.sequences.entry(new_channel_id).or_insert(0);
                let seq = *entry;
                *entry += 1;
                seq
            };
    ...
    fn rewrite_cdr_message(
        &mut self,
        mcap_writer: &mut mcap::Writer<BufWriter<File>>,
        msg: &crate::format::mcap::RawMessage,
        ...
        let decoder = crate::CdrDecoder::new();
        let decoded = match decoder.decode(schema, &msg.data, Some(&schema.name)) {
            ...
            Err(e) => {
                ...
                self.write_message_raw(mcap_writer, msg, channel_id, sequence)?;
                return Ok(());
            }
        };
...
mod tests {
    use super::*;
    use crate::format::mcap::transform::{TransformBuilder, TransformPipeline};
    ...
    fn temp_output(name: &str) -> PathBuf {
        let random = std::time::SystemTime::now()
            ...
            .unwrap()
            .subsec_nanos();
        std::env::temp_dir().join(format!("robocodec_mcap_test_{random}_{name}"))
    }
