...
use crate::format::mcap::RawMessage;
use crate::reader::{BagFormatReader, BagRawMessageIter, FormatReader};
use crate::rewriter::{FormatRewriter, RewriteOptions, RewriteStats};
use crate::schema::{parse_schema, MessageSchema};
use crate::CdrDecoder;
...
impl BagRewriter {
        ...
        P2: AsRef<Path>,
    {
        ...
        let reader = BagFormatReader::open(input_path.as_ref())?;
        ...
        if let Some(ref pipeline) = self.options.transforms {
            let transform_channels: Vec<crate::format::mcap::transform::ChannelInfo> = channels
                .values()
                .map(|ch| crate::format::mcap::transform::ChannelInfo {
                    id: ch.id,
    ...
    fn cache_schemas(&mut self, reader: &BagFormatReader) -> Result<()> {
        let pipeline = self.options.transforms.as_ref();
