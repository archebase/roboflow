        ...
        for result in stream {
            ...
            if let Some(type_str) = transformed_type {
                if let Some(schema) = schemas.get(type_str) {
                    // Convert io::RawMessage to format::RawMessage
                    let format_raw_msg = RawMessage {
                        channel_id: raw_msg.channel_id,
                        log_time: raw_msg.log_time,
                        publish_time: raw_msg.publish_time,
                        data: raw_msg.data.clone(),
                        sequence: raw_msg.sequence,
                    };
                    match self.rewrite_cdr_message(&cdr_decoder, &format_raw_msg, schema) {
                        Ok(data) => {
                            // Write re-encoded message
                            writer.write_message(&crate::format::writer::BagMessage::from_raw(
                                new_conn_id,
                                raw_msg.log_time,
                                data,
                            ))?;
