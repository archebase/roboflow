//! Kps metadata generation.
//!
//! Creates `meta/info.json` and other metadata files
//! required by the Kps dataset format.
...
pub fn write_info_json(
    ...
    for mapping in &config.mappings {
        let (shape, dtype) = match &mapping.mapping_type {
            super::config::MappingType::Image => {
            ...
            super::config::MappingType::State => {
            ...
            super::config::MappingType::Action => {
                if let Some(dim) = state_shapes.get(&mapping.topic) {
                    (vec![*dim], "float32")
                } else {
                    (vec![7], "float32")
            ...
            super::config::MappingType::Timestamp => (vec![1], "float64"),
            super::config::MappingType::OtherSensor => {
                // Other sensors typically have small dimensionality
                if let Some(dim) = state_shapes.get(&mapping.topic) {
                    (vec![*dim], "float32")
                } else {
                    (vec![3], "float32") // Default for IMU etc
                }
            }
            super::config::MappingType::Audio => {
                // Audio data - shape depends on configuration
                if let Some(dim) = state_shapes.get(&mapping.topic) {
                    (vec![*dim], "float32")
                } else {
                    (vec![48000], "float32") // Default 1s at 48kHz
                }
            }
        };
