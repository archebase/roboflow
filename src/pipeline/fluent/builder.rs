...
impl Robocodec<WithOutput> {
    ...
    pub fn run(self) -> Result<RunOutput> {
        ...
        if self.input_files.len() == 1 {
            ...
            let mut config = PipelineConfig::new(input_path, &resolved_output);
            config = config
            ...
            config.transform_pipeline = self.transform;

            let pipeline = AsyncPipeline::with_config(config)?;
            let report = pipeline.run()?;

            return Ok(RunOutput::Single(report));
        }
        ...
        for input_path in self.input_files.iter() {
            ...
            if use_hyper {
                ...
                // Use standard pipeline
                let mut config = PipelineConfig::new(input_path, &output_file);
                config = config
                    .with_compression_level(compression_level)
                    .with_chunk_size(chunk_size);
                ...
                config.transform_pipeline = self.transform.clone();

                let result = AsyncPipeline::with_config(config)
                    .and_then(|pipeline| pipeline.run())
                    .map(|report| {
                        FileResult::from_standard_success(
                            input_path.display().to_string(),
                            output_file.display().to_string(),
                            report,
                        )
                    ...
                    .unwrap_or_else(|e| {
                        error!(
                            ...
                            output = %output_file.display(),
                            error = %e,
                            "Failed to process file with standard pipeline"
                        );
                        FileResult::from_failure(
                            input_path.display().to_string(),
                            output_file.display().to_string(),
...
impl FileResult {
    ...
    pub fn success(&self) -> bool {
        matches!(
            self.result,
            FileResultData::StandardSuccess(_) | FileResultData::HyperSuccess(_)
        )
    }
    ...
    pub fn standard_report(&self) -> Option<&PipelineReport> {
        match &self.result {
    ...
    fn from_standard_success(
        ...
        Self {
            input_path,
            output_path,
            result: FileResultData::StandardSuccess(report),
        }
    }
