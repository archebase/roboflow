...
impl BagFormat {
    ...
    /// Create a BAG writer (not yet implemented).
    pub fn create_writer<P: AsRef<Path>>(_path: P) -> Result<Box<dyn FormatWriter>> {
        Err(CodecError::unsupported(
            "BAG writer not yet implemented in robocodec",
        ))
    }
}
...
impl ParallelBagReader {
    ...
    fn process_chunk(
        ...
        let bag_messages = parser.parse_chunk_messages(&decompressed, conn_id_map)?;

        // Convert BagMessageData to RawMessage
        let messages: Vec<RawMessage> = bag_messages
            .into_iter()
            .filter(|msg| channels.contains_key(&msg.channel_id))
            .map(|msg| RawMessage {
        ...
        Ok(ProcessedChunk {
            ...
            chunk_sequence: chunk_info.sequence as usize,
            message_start_time: chunk_info.start_time,
            message_end_time: chunk_info.end_time,
        })
    }
}
    ...
    fn read_parallel(
        ...
        for result in results {
            ...
            if !processed.messages.is_empty() {
                let mut chunk_data = MessageChunkData::new(processed.chunk_sequence as u64);
                chunk_data.message_start_time = processed.message_start_time;
                chunk_data.message_end_time = processed.message_end_time;
                for msg in processed.messages {
                    chunk_data.add_message(msg);
                }
                sender.send(chunk_data).map_err(|e| {
