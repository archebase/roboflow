...
fn test_reading_vs_full_pipeline() {
    ...
    println!("  Writer/encoding:     {:.2}s ({:.1}%)", overhead, overhead_pct);
    ...
    if overhead_pct > 50.0 {
        ...
        println!("     Encoding only takes {:.1}% of total time", overhead_pct);
    }
    println!("{}", std::iter::repeat("=").take(80).collect::<String>());
}
...
async fn run_full_pipeline_benchmark() -> f64 {
    ...
    let _stats = DatasetWriter::finalize(&mut writer)
